...
export function createScatterplot (selector, movies, onMovieFocus) {
  ...
  function drawLegend (metric, colorScale, plot) {
    ...
    const legendDomain = d3.extent(movies, d => d[metric.key])
    ...
    linearGradient.selectAll('stop')
      ...
      .append('stop')
      .attr('offset', d => `${d * 100}%`)
      .attr('stop-color', d => colorScale(legendDomain[0] + d * (legendDomain[1] - legendDomain[0])))

    const legendHeight = Math.min(430, plot.height - 24)
  ...
  function placeTooltip (event, movie, plot) {
    tooltip
  ...
  function render (metric) {
    ...
    const width = Math.max(760, bounds.width)
    const height = Math.max(620, bounds.height)
    ...
    thresholdLayer
      ...
      .attr('x1', xScale(0))
      .attr('y1', yScale(0))
      .attr('x2', xScale(d3.max(movies, d => d.budget)))
      .attr('y2', yScale(d3.max(movies, d => d.budget)))
      .attr('stroke', '#ff2b42')
    ...
    thresholdLayer
      .append('text')
      .attr('x', xScale(d3.max(movies, d => d.budget) * 0.95))
      .attr('y', yScale(d3.max(movies, d => d.budget)) - 8)
    ...
    const dots = pointsLayer.selectAll('circle')
      .data(movies, d => d.id)
